...
const styles = StyleSheet.create({
    ...
    companyName: {
        ...
        color: '#1a56db', // Blue-600 to match UI
        marginBottom: 4
    },
    companySub: {
        fontSize: 10,
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                <View style={styles.header}>
                    ...
                    <View style={styles.quotationDetails}>
                        ...
                        <View style={styles.detailRow}>
                            ...
                            <Text style={styles.detailValue}>{quotation.status}</Text>
                        </View>
                    </View>
                </View>
                ...
                <View style={styles.billTo}>
                    ...
                    {/* Add client address if available in future */}
                </View>
                ...
                <View style={styles.table}>
                    ...
                    {quotation.items?.map((item, index) => (
                        <View key={index} style={styles.tableRow}>
                            ...
                            <Text style={[styles.tableCell, styles.colCode]}>{item.item_code}</Text>
                ...
                <View style={styles.totalsSection}>
                    <View style={styles.totalsBox}>
                        ...
                        {quotation.taxes?.cgst?.amount > 0 && (
                            <View style={styles.totalRow}>
                                <Text style={styles.totalLabel}>CGST ({quotation.taxes.cgst.percentage}%)</Text>
                                <Text style={styles.totalValue}>{formatCurrency(quotation.taxes.cgst.amount)}</Text>
                            </View>
                        )}
                        {quotation.taxes?.sgst?.amount > 0 && (
                            <View style={styles.totalRow}>
                                <Text style={styles.totalLabel}>SGST ({quotation.taxes.sgst.percentage}%)</Text>
                                <Text style={styles.totalValue}>{formatCurrency(quotation.taxes.sgst.amount)}</Text>
                            </View>
                        )}
                        {quotation.taxes?.igst?.amount > 0 && (
                            <View style={styles.totalRow}>
                                <Text style={styles.totalLabel}>IGST ({quotation.taxes.igst.percentage}%)</Text>
                                <Text style={styles.totalValue}>{formatCurrency(quotation.taxes.igst.amount)}</Text>
                            </View>
                        )}
                ...
                <View style={styles.termsSection}>
                    <View style={styles.termCol}>
                        ...
                        <Text style={styles.termValue}>{quotation.payment_terms || "N/A"}</Text>
                    </View>
                    <View style={styles.termCol}>
                        <Text style={styles.termLabel}>Delivery Terms</Text>
                        <Text style={styles.termValue}>{quotation.delivery_terms || "N/A"}</Text>
                    </View>
                    <View style={styles.termCol}>
                        <Text style={styles.termLabel}>Remarks</Text>
                        <Text style={styles.termValue}>{quotation.remarks || "No remarks"}</Text>
                    </View>
                </View>
